...
export function AddClassDialog({ trigger }: { trigger?: React.ReactNode } = {}) {
  ...
  const { data: coursesResult, isFetching: coursesFetching } = useQuery(
    courseListOptions({ search: courseSearch || undefined })
  );
  const { data: teachersData, isFetching: teachersFetching } = useQuery(
    teacherOptionsQuery({ search: teacherSearch || undefined })
  );
  // Default selection comes from an unfiltered query so the form still has a
  // valid starting value if the user begins typing in the search before picking.
  const { data: defaultCoursesResult } = useQuery(courseListOptions({}));
  const { data: defaultTeachersData } = useQuery(teacherOptionsQuery({}));
  ...
  const initialValues = React.useMemo<ClassFormValues>(
    () => ({
      ...defaultValues,
      courseId: defaultCoursesResult?.data[0]?.id ?? '',
      teacherId: defaultTeachersData?.[0]?.id ?? ''
    }),
    [defaultCoursesResult, defaultTeachersData]
  );
  ...
  // Use either the filtered list (current search) or the unfiltered list so the
  // trigger still shows the picked label after a search excludes the selection.
  const selectedCourse =
    coursesResult?.data.find((c) => c.id === courseId) ??
    defaultCoursesResult?.data.find((c) => c.id === courseId);
  const selectedTeacher =
    teachersData?.find((tt) => tt.id === teacherId) ??
    defaultTeachersData?.find((tt) => tt.id === teacherId);
